#!/usr/bin/env node
/**
 * Standalone OAuth2 refresh-token helper for mcp-gtm-ga4.
 *
 * This MCP's PRIMARY auth model is a Google service account supplied at runtime
 * via GOOGLE_APPLICATION_CREDENTIALS (see README). This helper is the OPTIONAL
 * user-OAuth onboarding path for people who do NOT have a service account — it
 * mints a user refresh token and prints a ready-to-save `authorized_user`
 * credential file that plugs into the SAME GOOGLE_APPLICATION_CREDENTIALS
 * runtime path (googleapis' GoogleAuth accepts authorized_user keyfiles).
 *
 ...
 *      OAuth 2.0 Client ID of type "Desktop app". Enable the Tag Manager API
 *      and the Google Analytics Admin + Data APIs.
 *   2. export GOOGLE_CLIENT_ID=...  GOOGLE_CLIENT_SECRET=...
 *   3. node get-refresh-token.cjs
 *   4. Approve in the browser (as the Google account with GTM/GA4 access).
 *   5. Either export the printed GOOGLE_REFRESH_TOKEN=... OR save the printed
 *      authorized_user JSON to a file and point GOOGLE_APPLICATION_CREDENTIALS
 *      at it.
 *
 * The OAuth scope requested is read from this repo's config.json (oauth.scope),
 * falling back to the committed GTM/GA4 default — the SAME source the MCP's
 * runtime auth path uses (src/oauthScope.ts), so helper and runtime never drift.
 */
...
// The committed min-scope default MUST stay byte-identical to
// src/oauthScope.ts DEFAULT_GTM_GA4_SCOPE (parity-tested). GTM has write tools,
// so the edit + publish scopes are required; tagmanager.readonly is NOT
// requested (edit grants read; the runtime never asks for it).
const DEFAULT_GTM_GA4_SCOPE = [
...
// and src/index.ts runAuth). Google matches loopback redirect URIs on scheme +
// host + path and IGNORES the port, so onboarders register ONE pattern:
//   http://localhost/callback
// Use host `localhost` and path `/callback` everywhere.
...
/** Resolve oauth.scope from a parsed config object; falls back to the default. */
function resolveScopeFromConfig(config) {
  const scope =
    config && typeof config === "object" && config.oauth && typeof config.oauth.scope === "string"
      ? config.oauth.scope
      : "";
  return normalizeScope(scope) || DEFAULT_GTM_GA4_SCOPE;
}

/** Read oauth.scope from a config file on disk; falls back to the default. */
...
/** Build the authorized_user credential object googleapis' GoogleAuth accepts
 * as a GOOGLE_APPLICATION_CREDENTIALS keyfile. */
function buildAuthorizedUserCredential({ clientId, clientSecret, refreshToken }) {
  return {
...
async function run() {
  ...
  const port = Number(process.env.OAUTH_CALLBACK_PORT || 8095);
  ...
  const credential = buildAuthorizedUserCredential({
    clientId,
    clientSecret,
  ...
  process.stderr.write(
    "\nTo use the service-account runtime path instead, save this JSON to a file\n" +
